...
function Traditionalized(text) {
    const simplifiedChars = Autohan_JTPYStr(); // 簡體字元集合
    const traditionalChars = FTPYStr(); // 繁體字元集合
        ...
        if (node.nodeType === Node.TEXT_NODE) {
            ...
            if (newLanguage === "simplified") {
                node.textContent = isAutoHan ?
                    Traditionalized(node.textContent).replace(/簡/g, "繁") // AutoHan: 反向轉換並替換簡
                    :
                    Simplized(node.textContent); // 正常轉換為簡體
            } else if (newLanguage === "traditional") {
                node.textContent = isAutoHan ?
                    Simplized(node.textContent).replace(/繁/g, "簡") // AutoHan: 反向轉換並替換繁
                    :
